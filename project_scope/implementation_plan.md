...
**Step 5 — Silver** ✓ (shipped 2026-05-09; FDA `description` field-mapping correction deferred to the `feature/silver-field-remap` PR per `phase-6-execution-plan.md` § Sequencing Constraints)
...
| `extraction_runs` source-specific column sparsity | **Phase 7 prerequisite** (was gated on USCG forensics) | Pending — deferred. USCG returned 2026-05-15 and chose the pragmatic-capture path (see the 2026-05-16 update in the section below), so the original USCG-forensics blocker is resolved; no urgency replaced it. Revisit if Phase 7 cron-prep surfaces a real operational cost. See section below for full reopen condition. |
...
- **Execution of the history / lifecycle / cross-source SCD-2 deliverables below is owned by `project_scope/phase-6c-execution-plan.md`** (commits 6c.0–6c.8; it also absorbs NHTSA v1.5 Layer 2/3 from `silver_v15_migration_plan.md`). The descriptions here remain the master spec; that doc is the how.
...
- Framework ADR (**next free is 0036** per `documentation/decisions/README.md` — 0024/0025 reserved for Phase 8's API design and deployment-target ADRs; 0026–0033 filed; 0034/0035 filed as Proposed stubs)
