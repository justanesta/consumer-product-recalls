...
- **Status:** Active — planning complete, build not started. On branch `feature/phase-6c-history-lifecycle` (created off `main` at `3529e4c`; 0 commits ahead at plan time). **Structure: a sequence of staged commits 6c.0–6c.8 on this single branch, then one PR to `main`** — not a PR-per-rung ladder like 6b.
...
**Files — new.**
- `dbt/models/silver/recall_lifecycle.sql` — per identity: `first_seen_at = MIN(extraction_runs.started_at)`, `last_seen_at = MAX(...)`, `edit_count = COUNT(DISTINCT content_hash)` (all five sources, manifest-independent); `is_currently_active` (identity in the most-recent successful run's manifest) + `was_ever_retracted` (a successful run between first/last seen where the identity was absent) for **USDA** (manifest) and **NHTSA** (the flat file *is* a manifest — its identity set per run, ADR 0026 §NHTSA). For CPSC/FDA/USCG those two columns are NULL with a documented reason until their manifest lands (tracked follow-on; ADR 0026 source-parameterized design makes this additive).
