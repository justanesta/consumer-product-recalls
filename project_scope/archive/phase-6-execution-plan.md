...
- **Out of scope:**
  ...
  - **FDA null-`eventlmd` incremental blind spot — RESOLVED FAVORABLY by the 2026-06-01 census (no Phase 7 leg needed now).**
    ...
    "OPEN production-cadence question"). **Gate verdict (`seed_completeness_gate.sql` Q5–7, run 2026-06-01):**
    all **197** null-`eventlmd` rows are a structural **2000–2016** archive tail, and **Q6 = 0 recent**
    (none initiated since 2025-01-01). So no new recall currently arrives null → the `eventlmd`
    incremental is **sound as-is; no `recallinitiationdtfrom` watermark leg is needed.** Keep Q6 as a
    periodic monitor — if it ever returns recent null-`eventlmd` rows, add the (filterable)
    `recallinitiationdtfrom` leg and/or a periodic `filter:"[]"` backstop. All 197 nulls carry
    `recall_initiation_dt` (Q7), so that leg is viable should the contingency ever fire.
...
| **FDA** | `recalls deep-rescan fda --change-type=historical_seed` with **NO `--start-date/--end-date`** → full-corpus `filter:"[]"` path (`FdaDeepRescanLoader.set_full_corpus`). **NOT** a date window — a window silently drops 197 null-`EVENTLMD` rows (api_observations.md Finding P). **Prereq: migrations 0019 + 0020** (`alembic upgrade head`) — 0019 adds the §7a capture-expansion columns; 0020 drops NOT NULL on event_lmd/center_cd/product_type_short/firm_legal_nam so the 197 null-`EVENTLMD` rows land instead of silently quarantining. ~134,450 records, ~54 pages @ 2,500/page, 5s/page pacing ≈ ~5-6 min. **Mandatory post-seed gate:** `scripts/sql/fda/bronze/seed_completeness_gate.sql` (compare `count(distinct source_recall_id)` to the captured `RESULTCOUNT`; do not trust `status=success`). Implemented per `project_scope/archive/fda-historical-seed-plan.md`. **✓ COMPLETE 2026-06-02 → 134,450 distinct in bronze.** The 2026-06-01 run sorted on non-unique `recalleventid` and dropped ~245 via tie-boundary straddle (`diagnose_seed_straddle.py`: 245/245 straddle, 0 in-page); fixed by re-seeding sorted on the **unique `productid`** (`fix/fda-seed-productid-sort`) → +256 idempotent top-up, gate Q1 = RESULTCOUNT, **0 straddle** on the new payload. Null-tail clean (197, Q6=0 recent). | Fresh API extraction (single bulk POST, paginated) | ~1.5-3 GB (`codeinformation` max 205,424 chars/record) | Medium (Akamai — 5s/page pacing; text/html throttle → wait 30+ min, re-run, dedup-safe) |
...
| 3. FDA ✓ COMPLETE (re-seeded 2026-06-02) | `recalls deep-rescan fda --change-type=historical_seed` (**no `--start-date/--end-date`** → full-corpus `filter:"[]"` path; **not** a date window — a window silently drops the 197 null-`EVENTLMD` rows, Finding P) | **Apply migrations `0019` + `0020` first** (`alembic upgrade head`): 0019 (capture expansion) added the 11 §7a SHIP fields to bronze + `_DISPLAY_COLUMNS` so this one-time pull captures everything silver + 6b need (R2 replay can't recover un-requested columns); 0020 (core-fields nullable) lets the 197 null-`EVENTLMD` rows land instead of silently quarantining. `codeinformation` drops the page size 5000→2500, so the pull is ~2× the request count. No count guard on `FdaDeepRescanLoader`. Akamai posture: 5s/page pacing, off-peak, pause-and-resume if HTTP 204 / `text/html` throttle hit. **As-run (2026-06-01):** fetched 134,450 = RESULTCOUNT; 24 `check_date_sanity` rejects (year-typo `2013→0013`) reclaimed via `recalls recover-rejected fda` (PR #45); **inserted 134,181 → 134,205 distinct in bronze.** ✓ **RESOLVED 2026-06-02:** the 2026-06-01 run dropped ~245 distinct products (within_batch_dedup collapsed 245 `recalleventid` tie-boundary straddle dups; `diagnose_seed_straddle.py`: 245/245 straddle, 0 in-page; a 2nd fetch dropped a different 339). Fixed by sorting both deep-rescan paths on the **unique `productid`** (`fix/fda-seed-productid-sort`); re-seed 2026-06-02 (+256 top-up) → **134,450 distinct, 0 straddle** (gate Q1 = RESULTCOUNT). Quality clean (197 null tail, 0 recent-null leak — Phase 7 blind-spot resolved). |
...
> **Superseded 2026-06-06 by `project_scope//archive/phase-6c-execution-plan.md`** — the full, current Phase 6c scope (the three items below + the SCD work that was scattered across ADR 0035 §5, `archive/silver_v15_migration_plan.md`, and the W3 SCD audit, now consolidated as commits 6c.0–6c.8 on one branch + a single PR). The bullets below predate USCG reactivation, the 6b USCG SCD-2 build (#59), and the W3 audit; they survive here as the original seed only. **Build from the new doc.**
...
- **Exploration note (surfaced in 6e) — firm-location geography semantics.** `fct_recalls_by_geography`'s `firm_location` lens inherits a recall's state from the **6b cross-source canonical firm**. Empirically (6e spot-check) the inherited state is overwhelmingly the firm's **corporate HQ / registered (FDA-FEI) address** — Walmart→AR, Target→MN, 3M→MN, Amazon→WA, Honda→CA — **not** the product's manufacturing site or distribution geography. Two conceptual gaps to flesh out in the 6f gold textual summary: (1) a cross-source *collapsed* firm does not map to one physical building — a canonical firm can carry **multiple FDA FEIs** (Target has 3) and MICs across different facilities/states, and the lens spreads one recall across all of them; (2) the **FDA FEI conflation** — an FEI identifies an *establishment (facility)*, so merging firms by normalized name attaches facility addresses that may not be the recalling site, and is sensitive to firm-merge quality. Decide + document the intended semantics ("firm registered/HQ location" vs "production" vs "distribution"), and whether `firm_location` should prefer a single primary/registered address or be **relabelled** to say what it actually is. Distribution geography (`distribution` lens) is the cleaner answer to "where did the product go."
