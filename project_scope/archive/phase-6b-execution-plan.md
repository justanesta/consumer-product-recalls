...
- **Owning master plan:** `project_scope/implementation_plan.md` Phase 6. Supersedes the "Phase 6b — Firm Entity Resolution" section of `project_scope/archive/phase-6-execution-plan.md` (lines ~166-277), which was written BEFORE silver-remap PR #58 and carries stale `firm.sql` line references and stale "greenfield" assumptions.
...
> **As-built (2026-06-03) — gates run + D1 revised.** Gate verdicts: **G1c** single-shot seed ✓ (9,828, 1 row/id — bronze raw reads valid). **G1b** keep all three DBA forms (`dba` 418 / `d/b/a` 132 / `doing business as` 45) + the `(doing business as X)` paren form. **G1** geo strip = **comma-optional, anchored to the LAST `of` whose tail contains a US state/country (excl. `America`), guarded by an integral/narrative blocklist** (`city|king|division|subsidiary|...` + `, a/an <role> of` + `on behalf|owner|affiliate|formerly|business of` + `out of business`). Validated holistically on the full 550-name cohort (`data/exploratory/cpsc/g1_comma_less_cohort.csv`): **0 over-strips**; remaining misses are city-only (`of Seattle`) that RapidFuzz recovers via subset `token_set_ratio` — over-strips cannot be undone, so precision-first.
> **D1 REVISED (user decision):** the cleaning is a **Python enrichment module, NOT a dbt-SQL macro** — the geo-anchored + vocab + blocklist logic is ~clean in Python, gnarly as a `regexp_replace`. **Shipped:** `src/enrichment/firm_normalization.py` (`clean_firm_name` + `extract_firm_dba`; 71 pytest cases from real corpus strings; ruff/pyright/pytest green). Lockstep is preserved by having both `firm.sql` + `recall_event_firm.sql` read an IDENTICAL precomputed clean-name (vs the shared-macro plan). **Remaining 6b.1 work:** a `recalls` CLI step mapping distinct raw CPSC names → `(clean_name, extracted_dba)` into a table (grain + standalone-table-vs-fold-into-`firm_crosswalk` is the next decision), register as a dbt source, wire both firm models' CPSC branches to it, populate `firm.alternate_names`, stamp CPSC `match_confidence`, and **drop the `firm.normalized_name` unique test (critic C5)**.

**Scope (superseded by the as-built note above — kept for history).** Fill the `clean_firm_name`/`extract_firm_dba` macros with the deterministic CPSC suffix-strip (trailing `, of <geo>`, `, dba <name>`, `, doing business as <name>`) and DBA-brand capture; route CPSC `normalized_name`/`firm_id` through the macro in BOTH firm models (lockstep); populate `firm.alternate_names` from the extracted DBA; stamp CPSC `match_confidence` values on the bridge. Deterministic only — no Python, no rapidfuzz.
...
> **As-built (2026-06-03) — gates run + two divergences from the body below; see ADRs 0031/0033.**
> - **Increment A — `firm_fei_edges`:** built green, 15,668 edges (additive; 6b.4's clusterer consumes it, not wired into firm/bridge here).
> - **Increment B — AC DELCO:** the body's "`recall_product.sql:130` only" fix BREAKS the `recall_product_id` `unique` test — `stg_nhtsa_recalls`' DISTINCT-ON partitions on the 11-tuple incl. RAW `maketxt`, so `AC DELCO`+`ACDELCO` are TWO staging rows → one md5 → dup PK. Correct fix canonicalizes `maketxt` at BOTH the staging partition AND the md5 via a NEW `dbt/macros/normalize_maketxt.sql` (single source of truth; conservative whitespace+case = over-merge-safe). AC DELCO is ADR 0033's Normalization-class exemplar (the one class the NHTSA-product SCD cannot absorb on an anchor); 6b.3 discharges the whitespace/case sub-case deterministically (recorded as ADR 0033's 2026-06-03 amendment) + a forward drift monitor `assert_nhtsa_maketxt_drift_caught` (warn). Built green: recall_product 470,723 rows, `unique` holds, 321,540 NHTSA = 321,540 distinct ids; G8 28/28 same-make (0 over-merge); semi-aggressive normalize deferred to the 6a.5 full-corpus re-probe. G6/G7 captured (FDA FEI 14,285; NHTSA 3,940 firms, 95.9% disjoint).
> - **Increment C — NHTSA cleaning = C-lite.** Post-D1 there is no clean macro; NHTSA cleaning flows through the source-agnostic `firm_crosswalk` (global PK firm_id), which forces ONE unified cleaner. Rather than re-touch the validated CPSC crosswalk mid-6b.3, this PR ships ONLY the deterministic `clean_nhtsa_firm_name` + tests (validated on the full 3,940-name dump: 0 over-strips, 10 merge clusters 21→10). The crosswalk wiring + cross-source unification + source-agnostic `match_confidence` + the CPSC non-DBA-paren probe are DEFERRED to PR 6b.4 (see its "Deferred from PR 6b.3" note). **NHTSA firm_ids change at 6b.4, not 6b.3.** The Scope/Files body below ("route NHTSA through the shared `clean_firm_name` macro") predates the 6b.1 D1 revision — STALE on the venue (Python crosswalk, not a dbt macro).
...
> - **Increment 2 SHIPS at NAME/BRAND GRAIN; FEI is attribute-only (ADR 0037, finalized 2026-06-05).** Path: first clusterer (subset⇒merge + any-shared-FEI) over-merged → replaced by a 3-tier model (2026-06-04) → **FEI Tier-0 then pulled out** when the validation run produced cross-corporate blobs (Biomat=CSL+Grifols+Takeda+Octapharma) via cross-FEI name-bridging that no FDA signal can untangle (surviving-name 7%/0%-on-bridges). **Shipped `firm_resolution.py`: Tier 1** name repair (identical distinctive set / `token_sort_ratio` typo, always on) **+ Tier 2** optional `--rollup` entity rollup (≥2 shared distinctive tokens, place-denylisted via `place_words.py`). **Tier 0 FEI (`fei_resolve`) retained + tested but OFF** (`--fei-merge`, default off) — FEI rides on `firm.observed_company_ids`, uniform with CPSC/USDA/USCG structured ids. Grain = **one firm = one brand/name cluster**, uniform across all 5 sources. Wiring: CLI `--rollup/--no-rollup` + `--fei-merge/--no-fei-merge` + `--rollup-threshold`; resolver_version `allsrc-tier{12|1|012}-roll90-v3`; `_silver.yml` enum (`fei_exact` reserved/opt-in); NEW `diagnose_fei_fanout.sql` + `measure_surviving_coverage.sql`. Default run no longer needs `firm_fei_edges`. **Measured (FEI off): 28,599 names → 24,204 firms, no blobs.** NO firm.sql/migration/lockstep change. Prereq: `dbt build --select staging`. **The crosswalk in the DB is the OLD FEI-on output — re-resolve before building silver firm. Version bump 0.14.1→0.15.0 pending.**
...
**Deferred from PR 6b.3 (C-lite, 2026-06-03) — fold into THIS PR.** 6b.3 shipped the deterministic NHTSA cleaner `clean_nhtsa_firm_name` (+ the reusable `_strip_parentheticals` helper) in `src/enrichment/firm_normalization.py`, validated on the full 3,940-name corpus (`data/exploratory/nhtsa/name_normalization_features.csv`: 0 over-strips; 10 merge clusters, 21→10 names) but NOT wired into any firm_id. This PR consumes it:
- **Unified cleaner:** `_strip_parentheticals(clean_firm_name(raw))` — the CPSC `clean_firm_name` stays UNTOUCHED (its 71 tests hold); the wrapper adds general balanced-paren strip for EVERY source. One cleaner is mandatory because `firm_crosswalk.firm_id` is a global PK (one row per `md5(raw)`); two source-specific cleaners + dedup is the dual-oracle-near-dedup trap and is rejected.
- **NHTSA names into the crosswalk:** extend `crosswalk_writer.py` with a distinct-NHTSA-names query (`mfgname ∪ mfgtxt`) and UNION it with the CPSC names so each distinct raw name yields exactly ONE crosswalk row (no per-source duplicate firm_id).
- **Source-agnostic `match_confidence`:** a crosswalk row is no longer source-tagged, so rename `cpsc_suffix_strip_exact`→`geo_suffix_strip_exact`, `cpsc_dba_extract_exact`→`dba_extract_exact`, and add `paren_strip_exact` (`_silver.yml` enum + `crosswalk_writer` labels; the enum is warn-level → non-breaking).
- **CPSC blast-radius gate (NEW):** `scripts/sql/cpsc/bronze/probe_non_dba_parens.sql` — the wrapper newly strips non-DBA parens from CPSC names too (strict improvement); size + eyeball before committing.
- **Corp-form (69.5%) + regional (5.7%) NHTSA tokens** are NOT deterministically cleaned — they become RapidFuzz scoring stopwords here (the G0 SHENZHEN/CO/LTD watch-item). NHTSA firm_ids change at THIS PR, not 6b.3.
...
**Scope.** Convert `firm_manufacturer_attributes` from a Type-1 listing-only sidecar to the current-view over a new dbt SCD-2 snapshot built from the (currently unconsumed) detail staging view; add the time-sensitive attribution flag on the bridge. This is BOTH a source-switch (listing → detail) AND new snapshot infra. v1 attributes to the current MIC holder but FLAGS the ambiguity; the as-of-BUILD-date (HIN) join is deferred (only ~13/221 OOB years parseable, confirmed-in-doc).

**Confirmed in-scope for 6b (2026-06-03).** The USCG SCD-2 silver model is correctly placed here, not deferred to another stage: `archive/silver_v15_migration_plan.md:216,220` assigns it to "Phase 6 work … ADR 0035," **"deliberately kept off the detail-capture branch (would otherwise collide with Phase 6b on `firm.sql`)"**; and `scd_field_designations.md:33` records the USCG firm anchor as the corpus's **only Type-2 NEED**, MEASURED + monitor-confirmed (365 prior / 221 OOB-recycled — paren 205 + dash-form 16, OOB regex broadened 2026-06-05 — of 718 recalled MICs). The bronze prerequisite (`feature/uscg-manufacturers-detail-addition`) has already landed (migrations 0017/0018, `stg_uscg_manufacturer_details`). Only the *cross-source* SCD-2 policy (CPSC/FDA/USDA) stays out of 6b (post-Layer-3).
...
- Script: `scripts/sql/cross_source/scd_monitors/assert_mic_holder_stable.sql` (EXISTS — re-run Q2-Q4, capture to `documentation/uscg/`). Expected: 221 OOB-recycled (205 paren + 16 dash-form `- OOB`, broadened 2026-06-05) / 365 prior / 718 recalled. The bridge flag stamps the broad **365** prior-holder set (precision-over-recall); the 221 OOB + the Q4 sample are the high-confidence subset.
