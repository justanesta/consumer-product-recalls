...
| W4 | **Silver/staging SQL remap** — Phase A (staging projections) → B (`recall_event`, incl. FDA Bug 1) → C (`recall_product`, USDA/NHTSA `type` bugs + USCG `model` bug) → D (`firm`+`recall_event_firm`: CPSC Retailers-out Option B + NHTSA filer/mfr split) → E (`dbas` element filter). Green `dbt build --select <model>+` per commit | 🟡 **Phase A done** (USDA `company_media_contact`; USCG `severity`/`hin`/`model_year`/`boat_type` sentinels; NHTSA `model_year` from `yeartxt`; + consolidation §2 `model_year` cross-source gap-fix) — awaiting green build before B |
