...
| **FDA** | `recalls deep-rescan fda --change-type=historical_seed` with **NO `--start-date/--end-date`** → full-corpus `filter:"[]"` path (`FdaDeepRescanLoader.set_full_corpus`). **NOT** a date window — a window silently drops 197 null-`EVENTLMD` rows (api_observations.md Finding P). **Prereq: migrations 0019 + 0020** (`alembic upgrade head`) — 0019 adds the §7a capture-expansion columns; 0020 drops NOT NULL on event_lmd/center_cd/product_type_short/firm_legal_nam so the 197 null-`EVENTLMD` rows land instead of silently quarantining. ~134,450 records, ~54 pages @ 2,500/page, 5s/page pacing ≈ ~5-6 min. **Mandatory post-seed gate:** `scripts/sql/fda/bronze/seed_completeness_gate.sql` (compare `count(distinct source_recall_id)` to the captured `RESULTCOUNT`; do not trust `status=success`). Implemented per `project_scope/fda-historical-seed-plan.md`. **Run 2026-06-01 → 134,205 distinct in bronze** (134,181 inserted + 24 reclaimed). ⚠️ **Likely ~245 short:** seed fetched 134,450 = RESULTCOUNT, but `within_batch_dedup` collapsed **245 duplicate rows** from `recalleventid` tie-boundary straddle (log: invariants-passing 134,426 → inserted 134,181); `productid` is unique + fetched = RESULTCOUNT ⟹ ~245 distinct products *dropped* (plan §8 "tie-boundary DROP"). A 2026-06-01 re-probe still reads 134,450 → **not drift.** Otherwise clean (0 post-insert dups; 197 null-`eventlmd` rows captured; 0 recent-null leak). **Fix: re-seed `sort=productid`** (unique → no straddle; plan §12; probe confirms the sort works) on the 6a.5 branch, then re-gate. | Fresh API extraction (single bulk POST, paginated) | ~1.5-3 GB (`codeinformation` max 205,424 chars/record) | Medium (Akamai — 5s/page pacing; text/html throttle → wait 30+ min, re-run, dedup-safe) |
