...
| **FDA** | `recalls deep-rescan fda --change-type=historical_seed` with **NO `--start-date/--end-date`** → full-corpus `filter:"[]"` path (`FdaDeepRescanLoader.set_full_corpus`). **NOT** a date window — a window silently drops 197 null-`EVENTLMD` rows (api_observations.md Finding P). **Prereq: migrations 0019 + 0020** (`alembic upgrade head`) — 0019 adds the §7a capture-expansion columns; 0020 drops NOT NULL on event_lmd/center_cd/product_type_short/firm_legal_nam so the 197 null-`EVENTLMD` rows land instead of silently quarantining. ~134,450 records, ~54 pages @ 2,500/page, 5s/page pacing ≈ ~5-6 min. **Mandatory post-seed gate:** `scripts/sql/fda/bronze/seed_completeness_gate.sql` (compare `count(distinct source_recall_id)` to the captured `RESULTCOUNT`; do not trust `status=success`). Implemented per `project_scope/fda-historical-seed-plan.md`. **Run 2026-06-01 → 134,205 distinct in bronze** (134,181 inserted + 24 reclaimed). ⚠️ **CONFIRMED ~245 dropped** (`diagnose_seed_straddle.py`, 2026-06-01: **245/245 dups are page-boundary straddles, 0 in-page** → all tie-boundary re-reads, not source dups; a 2nd independent fetch dropped a *different* 339). Fetched 134,450 = RESULTCOUNT ⟹ ~245 distinct products lost; re-probe still 134,450 → not drift. Null-tail clean (197 captured, 0 recent leak). **Fix: re-seed `sort=productid`** (unique → no straddle; plan §12; idempotent top-up) on a fresh branch off `main`, then re-gate. | Fresh API extraction (single bulk POST, paginated) | ~1.5-3 GB (`codeinformation` max 205,424 chars/record) | Medium (Akamai — 5s/page pacing; text/html throttle → wait 30+ min, re-run, dedup-safe) |
