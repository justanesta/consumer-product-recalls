...
- **Status:** **Complete (in-scope work shipped via Phase 6b–6e, PRs #59–#62).** (A) the `firm_fda_attributes` sidecar + `recall_event` posted-internet date and (C) the FDA Tier-3 press-release extractor (`src/extractors/fda_press_release.py`) + bronze child + `recall_event_press_release` silver child all landed. Tier-2 stayed **excluded** (settled skip); `code_information` → `recall_product_code` remains deferred to `freetext-enrichment-backlog.md`. Sibling to the merged `silver-field-remap-plan.md` (the "(a) PR"). *(Archive pending — phase-7 plan C36.)*
