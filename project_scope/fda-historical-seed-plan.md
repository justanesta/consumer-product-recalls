...
- **Status:** ✅ Code/tooling **SHIPPED** (PR #44 — migrations 0019/0020, `set_full_corpus`, per-page pacing/retry, dbt warn-tripwire + silver coalesce, gate SQL, tests). ✅ Seed **RUN 2026-06-01** → **134,205 distinct in bronze** (134,181 inserted + 24 `check_date_sanity` rows reclaimed via `recalls recover-rejected fda`, PR #45). ⚠️ **Likely ~245 incomplete:** `within_batch_dedup` collapsed **245 `recalleventid` tie-boundary straddle dups** (log: invariants-passing 134,426 → inserted 134,181); since `productid` is unique and fetched (134,450) = RESULTCOUNT, ~245 distinct products were *dropped* (§8 "tie-boundary DROP"). A 2026-06-01 re-probe still reads 134,450 → **not corpus drift.** Null-tail quality clean (197 captured, 0 recent leak — Phase 7 blind-spot still resolved). **Fix (do next, on the 6a.5 branch): apply the §12 hardening — re-seed with `sort=productid`** (unique key → no straddle; probe confirms the sort is valid), re-gate (expect distinct ≈ 134,450), then archive.
...
- [x] (user) `alembic upgrade head` → seed → `dbt build` → gate — **done 2026-06-01** (134,205 distinct; 24 reclaimed via `recalls recover-rejected fda`). Gate clean on dups/null-tail/recent-leak. ⚠️ **Action needed:** the 245 shortfall vs RESULTCOUNT is `recalleventid` straddle drops (not drift — a 2026-06-01 re-probe still reads 134,450) → **re-seed with `sort=productid`** (§12 hardening) and re-gate to capture the dropped ~245.
