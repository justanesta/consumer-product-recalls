...
> - **Increment 2 RE-DESIGNED as 3 TIERS, BUILT + GATES GREEN 2026-06-04** (awaiting user DB run). The first clusterer (subset⇒merge + DF hub-guard + any-shared-FEI must-link) catastrophically over-merged (WHOLE FOODS=Stryker, TRUE/SUN/K&B hubs) and was replaced — detail in **ADR 0037 tiered amendment** + memory [[phase_6b_plan_and_decisions]]. New `firm_resolution.py`: **Tier 0** FDA current-FEI grouping (`coalesce(surviving_fei,fei)`, fan-out gated — FEI is establishment-grain, not a firm key), **Tier 1** name repair (identical distinctive set / `token_sort_ratio` typo, always on), **Tier 2** optional `--rollup` entity rollup (≥2 shared distinctive tokens, place-denylisted via new `place_words.py`). Wiring: `firm_fei_edges.sql` += `current_fei`/`current_name`; `apply_clustering(rollup, fei_resolve)`; CLI `--rollup/--no-rollup` + `--rollup-threshold`; `_silver.yml` enum → `fei_exact`/`name_variant_exact`/`name_typo_high`/`rapidfuzz_rollup`/`singleton`; new `diagnose_fei_fanout.sql`. Measured: Tier 0+1 ~6,016 names consolidated / 17 clusters≥6 all legit; Tier 2 +~1,339 (~90% precise pre-denylist). NO firm.sql/migration/lockstep change. Prereq: `dbt build --select staging firm_fei_edges`. **The crosswalk in the DB is the OLD bad output — re-resolve before building silver firm. Version bump 0.14.1→0.15.0 pending.**
