...
| `cpsc/bronze/diagnose_products_array_append_only_violations.sql` | 7-query drill-down for when the `assert_cpsc_products_array_append_only` dbt test returns > 0: reproduces the count (Q1), discriminates a real reorder/insert/delete from the superseded predicate's within-snapshot-duplicate false positive (Q2), shows slot-content churn as triage context (Q3, *not* a conflation count — duplicates the warn-level name/model monitor), sizes the misfire population and its backlog (Q4), dates the tripping snapshot (Q5), verifies the replacement predicate's two classes — `LENGTH_REGRESSION` (Q6) and `ORDINAL_MOVED` (Q7) — against the live corpus, then follows through on a non-zero Q6 with the shape of the collapse (Q8) and the silver blast radius vs. `stg_cpsc_recalls`' latest-snapshot collapse (Q9) | ADR 0031 amendments 2026-06-13 / 2026-09-12; `dbt/models/silver/recall_product.sql:57-66` |
