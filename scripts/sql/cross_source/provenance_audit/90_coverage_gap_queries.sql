...
-- ----------------------------------------------------------------------------------
-- event_type_not_null_single_value   [enum_domain]   target: silver.recall_event.event_type
...
select event_type,
       count(*)                                   as n_rows,
       count(*) filter (where event_type is null) as n_null
from silver.recall_event
group by event_type
...
-- ----------------------------------------------------------------------------------
-- recall_event_id_md5_recipe   [key_recipe]   target: silver.recall_event.recall_event_id
...
-- ----------------------------------------------------------------------------------
-- distribution_states_vs_state_codes_non_conflation   [type]   target: gold.mart_recall_summary
...
-- ----------------------------------------------------------------------------------
-- mart_distribution_arrays_null_vs_empty   [nullability]   target: gold.mart_recall_summary
...
-- ----------------------------------------------------------------------------------
-- press_release_grain_key_and_release_type   [grain + enum_domain]   target: silver.recall_event_press_release
...
from silver.recall_event_press_release;
...
-- ----------------------------------------------------------------------------------
-- recall_event_history_intentional_empty_present   [normalization]   target: silver.recall_event_history
...
from silver.recall_event_history;
...
--           Confirm the crosswalk table name/schema (silver.firm_crosswalk?) before running.
...
from silver.firm_crosswalk            -- [CONFIRM NAMES]
