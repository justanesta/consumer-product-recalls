...
3. ~~**HTTP cache validators.** Inspect the existing cassettes for `ETag` / `Last-Modified` headers. If present, they could short-circuit the archive-migration re-ingest work currently handled by content hashing.~~ **Resolved 2026-05-10: not present, not coming.** Inspection of `tests/fixtures/cassettes/cpsc/test_happy_path_recent.yaml` (response headers at line 65333) confirms the CPSC API emits no `ETag` and no `Last-Modified`. The server actively opts *out* of HTTP caching by returning `Cache-Control: no-cache`, `Pragma: no-cache`, and `Expires` set equal to the `Date` header (i.e., "expires at the moment of issue"). Content-hash dedup remains the right idempotency mechanism for archive-migration re-ingests; the conditional-GET optimization that USDA uses (Finding P in `documentation/usda/recall_api_observations.md`) is structurally unavailable here. The `extraction_runs.response_etag` column will remain NULL for `source='cpsc'` by design — that's the correct state, not a wiring bug.
