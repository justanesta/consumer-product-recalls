...
7. **`products[].Type` and `products[].CategoryID` — KEEP in silver (decision reversed post-SQL 2026-05-29).** Cassette had falsely suggested both empty; 2026-05-29 SQL Q2 confirmed **both are populated for 58.8% of recalls** (596 of 1,422). They carry real categorization signal and stay as `recall_product.type` + `recall_product.category_id` columns. Separately: `products[].Description` and `products[].Model` ARE confirmed 100% empty across the corpus. Document those two as "documented-empty-by-source for CPSC" in `_silver.yml`; the CPSC branch of `recall_product` can either keep them as silently-blank for cross-source schema parity OR explicit-NULL them. Recommend silent-blank (zero code change) since downstream consumers can filter `WHERE product_description != ''` if needed.
   - **⚠ SUPERSEDED 2026-06-17 — reversed to explicit-NULL (branch `fix/empty-string-out-of-silver-gold`).** The "silent-blank (zero code change)" recommendation is no longer the project stance. `recall_product.sql` now normalizes empty-string free-text → NULL **source-uniformly** (the `normalized` CTE at the union output), so CPSC `description`/`model` are explicit NULL, not `''` — and the documented-empty-by-source note IS in `_silver.yml` as recommended. **Why the reversal:** (1) "let consumers filter `WHERE product_description != ''`" is exactly the *"downstream has to remember the dance"* anti-pattern **ADR 0027** rejects; (2) 3 of 5 sources (USDA/FDA/USCG) already `nullif` at staging, so silent-blank left CPSC/NHTSA as the lone inconsistency; (3) the **live API serving layer** (Phase 8) makes cross-source NULL-consistency a *contract*, not a nicety — the API should never branch on `'' vs NULL` per source. Empirically confirmed by `scripts/sql/cross_source/empty_string_freetext_audit.sql` (2026-06-17): CPSC `description` and `model` were **11,834 / 11,834 = 100% `''`** pre-fix; `type`/`category_id` 40.4% and `number_of_units` 31.7%. See TODO "Performance" §2.
