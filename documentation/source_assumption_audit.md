...
### C2 — `products[]` array is append-only — **FALSIFIED 2026-09-12**
...
| **Evidence today** | **FALSIFIED 2026-09-12.** CPSC is collapsing enumerated per-model product rows into a single summary row across the historical archive, walking forward roughly daily since 2026-06-10 (`00176` 8→1, `00119` 7→1, `00105` 6→1). Baseline: **64 regressions / 64 recalls / 107 product rows absent from silver** (of 10,005 recalls). Upstream, not an extraction artifact — the 2026-09-06 deep rescan re-fetched the collapsed arrays independently. ~~**Validated at scale 2026-06-13**~~ — that reading was an artifact of an unsound predicate (it evaluated "spans > 1 ordinal" and "spans > 1 snapshot" independently, so a consolidation that renames slot 1 as it truncates left one landing path on each side and was filtered out), **not** a clean corpus. The same predicate fired on 12 recalls where nothing had moved, holding `transform.yml` red 2026-09-07 → 2026-09-12. See `cpsc/array_stability_findings.md` → "C2 FALSIFIED — product-array consolidation wave (2026-09-12)". |
| **Falsification script** | `scripts/sql/cpsc/bronze/assert_products_array_append_only.sql` (Q1/Q2 shrinkage, Q4/Q5 slot movement) + `diagnose_products_array_append_only_violations.sql` (9-query drill-down) + dbt `assert_cpsc_products_array_append_only.sql` and `assert_cpsc_product_ordinal_stable.sql` |
| **ADR 0031 threshold** | **Split 2026-09-12** into two metered classes. **LENGTH_REGRESSION** (`products[]` shrank): assumption is falsified, so this is a *meter* not a gate — `warn_if '>0'` / `error_if '>64'`, baseline 64 and rising; raising the ceiling requires a fresh diagnostic run + doc update in the same commit. **ORDINAL_MOVED** (a singleton `(name, model)` changed slot): **any non-zero**, `severity=error`, baseline 0 — still a hard gate, because this is the class that **conflates** (a later product inherits an earlier slot's id). |
| **Downstream impact** | **Realised, not hypothetical.** Because `recall_product` reads only the latest snapshot via `stg_cpsc_recalls`, the 107 consolidated-away products no longer exist in silver and their `recall_product_id` values are orphaned for any consumer that cached them. Bronze retains every version (ADR 0007), so this is recoverable as a modelling decision — **open**, filed as TODO → Data Shape/Quality → "CPSC product-array consolidation"; interacts with the deferred CPSC product-grain SCD-2 history ("Move 2"). For the ORDINAL_MOVED class the original remedy stands: re-key / deterministic in-slot tiebreaker + rebuild silver from bronze. **Supersedes** the prior "switch to a content-based surrogate" fix — the project deliberately moved content *out* of the key for id durability. Affects the `cpsc_products` CTE in `recall_product.sql`. |
