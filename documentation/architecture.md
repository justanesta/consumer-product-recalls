...
The exact `firm_crosswalk` / `firm` / `recall_event_firm` columns and the `match_confidence` vocabulary live in [data_schemas.md](data_schemas.md#firm-resolution); the operator review loop (the `verify_fuzzy_clusters.sql` eyeball, the place-denylist refinement) is in [operations.md](operations.md#firm-resolution-recalls-resolve-firms).
