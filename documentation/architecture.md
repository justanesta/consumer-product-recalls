...
flowchart LR
    ...
    SV["Silver · dbt (staging → silver)<br/>recall_event/product · history · lifecycle · distribution_area<br/>firm cluster (+ Python firm_crosswalk + quantity_crosswalk, ADR 0037/C13)<br/>SCD-2 snapshots: firm_*_attributes (silver_snapshots)"]
    GD["Gold · dbt<br/>dim_date · mart_* serving · fct_* aggregates<br/>(incl. fct_recalls_by_country · fct_units_recalled w/ qty)"]
...
1. **Run metadata is recorded.** `extraction_runs` gets a row with `source`, `started_at`, `status`, `records_extracted`, `records_inserted`, and `change_type` (per [ADR 0027](decisions/0027-bronze-storage-forced-transforms-only.md), distinguishing routine runs from re-baselines). For presence-tracked sources (**USDA + NHTSA**, per [ADR 0026](decisions/0026-lifecycle-tracking-snapshot-presence-manifest.md) amended C16; NHTSA manifest written only on full-enumerating deep-rescan, keyed on `campno`), the same `_record_run` transaction also writes the per-run **presence manifest** (`extraction_run_identities`) — one row per recall identity returned by the run. Presence is distinct from content: bronze records *what changed*, the manifest records *what was present*, so silver can tell a retraction (absent upstream) from an unchanged record — both of which produce zero new bronze rows. The manifest is written here, not in `load_bronze`, because it FKs to the `extraction_runs` row, which is recorded after the bronze write.
...
| `silver/recall_event_history.sql` | Field-level edit history (ADR 0022 / 6c.1) — `LAG()` over **all** bronze snapshots (not the latest-only staging views), excluding re-baseline waves (ADR 0027). The fact-history half of Phase 6c; the dimension half is the SCD-2 snapshots in `silver_snapshots`. New Phase 7 columns on `recall_event`: `terminated_year` (USDA + USCG year-grain close), `reason_category_tokens` (USDA jsonb token array from `reason_category`). New on `recall_product`: `processing_categories` (USDA jsonb token array), `quantity_value`/`quantity_unit`/`quantity_category`/`quantity_basis` (C13 parsed from `number_of_units` via `quantity_crosswalk`). See [`data_schemas.md`](data_schemas.md#new-and-extended-silver-columns-phase-7). |
...
| `gold/dim_date.sql` | Generated calendar dimension — 1940-01-01 .. (current year + 2, dynamic); one row per day; `date_day` unique key. Replaces inline `date_trunc` in the **five** date-grained `fct_*` models (by_month / by_year / by_week / monthly_trend / fct_units_recalled). See [`data_schemas.md`](data_schemas.md). |
| `gold/mart_*.sql` (serving) + `gold/fct_*.sql` (aggregates) | Consumer-shaped gold (ADR 0038): denormalized serving marts for the API + pre-aggregated dashboards + the FTS product-search index. New in Phase 7: `fct_recalls_by_country` (per ISO-3166-1 alpha-2 country × source, FDA+USDA; US cell derived from distribution scope — not stored in silver); `fct_units_recalled` extended with FDA/USDA parsed quantities (via `quantity_crosswalk`). See [`gold_design_notes.md`](gold_design_notes.md) + [`data_schemas.md`](data_schemas.md). |
...
| `quantity.py` | Pure **free-text quantity parser** (Phase 7, C13) — `parse_quantity(raw)` → `ParsedQuantity(value, unit, category, basis)`. Precision-scoped: emits a value only for clean single-quantity shapes (~70% value+unit, ~17% value-only); messy ~10% (lot ids, list indices, code prefixes, N/M compounds, multi-product breakdowns) → NULL. Unit taxonomy maps synonyms to `(canonical_unit, category)` where `category ∈ {count, weight, volume, grouping}`. No I/O. |
| `quantity_crosswalk_writer.py` | I/O boundary for quantity enrichment — reads distinct FDA + USDA raw quantity strings from `stg_*` views, calls `parse_quantity` for each, truncate-reloads `quantity_crosswalk` (migration 0032, registered as `enrichment.quantity_crosswalk`). Mirrors `crosswalk_writer.py`'s pure/IO separation. Invoked by `recalls parse-quantities`. |
