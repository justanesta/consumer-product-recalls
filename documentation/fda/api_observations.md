...
#### N.1 Addendum (2026-06-08): two distinct HTML responses — a permanent fingerprint block vs. a transient 503 origin-error page

The overnight press-release seed (`FdaPressReleaseCheckpointedSeedLoader`, ~50.5K per-event GETs) ran cleanly for ~2 h 22 min (28 batches, steady ~49 req/min), then died on a single **`HTTP 503` whose body was `text/html`**, not JSON. This exposed that finding N's "treat *any* HTML response as stop-and-wait" conflated two genuinely different failures:
...
- **5xx (e.g. 503) *with* an HTML body — a transient origin error:** the captured 503 body (verified 2026-06-08 from R2, event 91085) is a **cached Akamai NetStorage "Accessdata Error" failover page** — `server: AkamaiNetStorage`, a 2022 `etag`, `cache-control: max-age=231`, **no `Retry-After`**, no abuse/excessive-requests wording. That is Akamai's canned error page, returned when the iRES *origin application* hiccups, **not** a per-client throttle. It is the textbook **transient** signal, indistinguishable in intent from a plain `503` (which the extractor already retried). The 2 h+ of clean, constant-rate operation beforehand also *rules out a simple per-minute rate ceiling* (finding N's abuse throttle trips in ~90 s at ~33 req/min — this ran 2 h 22 min at ~49 req/min before failing). Aborting the sweep on one of these is wrong; **the re-run replayed the identical batch (incl. event 91085) clean, confirming recovery.**
