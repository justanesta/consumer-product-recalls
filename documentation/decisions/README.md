...
- [0026 — Lifecycle tracking via per-run snapshot-presence manifest](0026-lifecycle-tracking-snapshot-presence-manifest.md) — closes bronze's retraction gap; USDA+NHTSA (campno key; deep-rescan-only write for NHTSA), separate `extraction_run_identities` table *(amended 2026-06-09: NHTSA track_presence enabled)*
...
- [0031 — Silver-row fragmentation strategy](0031-silver-row-fragmentation-strategy.md) — per-source surrogate keys, drift detection, reconciliation tiers *(amended 2026-06-06: NHTSA recipe migrated to md5(7-tuple) per ADR 0034; amended 2026-06-13: CPSC recipe migrated to stable md5(CPSC|source_recall_id|product_ordinal), append-only assertion escalated to error)*
...
- [0033 — Silver row versioning via SCD on stable anchor](0033-silver-row-versioning-via-scd-on-stable-anchor.md) — NHTSA `recall_product` 7-tuple stable anchor + Type-1 latest-wins + Type-2 dbt snapshot; framework generalizes cross-source
...
- [0013 — Error handling: retries, idempotency, and quarantine](0013-error-handling-retries-idempotency-and-quarantine.md) *(amended 2026-05-01: FDA HTML-redirect throttling; amended 2026-06-09: restricted-role mutation guard on `*_rejected` tables)*
