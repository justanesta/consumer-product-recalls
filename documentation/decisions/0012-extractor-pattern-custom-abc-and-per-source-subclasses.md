...
### Implementation notes — source-config loader and registry (Wave 2, landed 2026-05-10)

Implementation lives in:

- `src/config/source_registry.py` — Pydantic discriminated-union models (`RestApiSourceConfig`, `FlatFileSourceConfig`, keyed on the `source_type` literal), plus two static dicts: `EXTRACTOR_BY_SOURCE_NAME` (5 entries, routine extract path) and `DEEP_RESCAN_BY_SOURCE_NAME` (3 entries, deep-rescan path). Direct Python imports per ADR's "direct Python imports are simpler than runtime dispatch at this scale" line. Tests that need to mock a specific extractor class do so via `patch.dict("src.config.source_registry.EXTRACTOR_BY_SOURCE_NAME", {"cpsc": mock_cls})` — see `tests/cli/test_main.py` `_patch_extractor` / `_patch_deep_rescan` helper context managers.
- `src/config/source_loader.py` — `load_source_config(source_name)` reads `config/sources/<source>.yaml` and validates through `TypeAdapter[SourceConfig]`. Strict-fail under `extra="forbid"`.
- `src/cli/main.py` — both `extract` and `deep-rescan` commands collapse from per-source if/elif dispatch to a single registry-lookup pattern. Source-specific post-construction logic (etag_audit override, `--lookback-days` handler, NHTSA `--since`, FDA deep-rescan `set_date_range`) lives in the CLI by design — the loader handles config materialization only.

**Wire scope (5 fields).** Only `base_url` / `file_url`, `timeout_seconds`, `rate_limit_rps`, `etag_enabled` (REST sources only), and `historical_seed_urls` (NHTSA deep-rescan only) flow from YAML to extractor constructors. The other YAML fields (`incremental_filter_param`, `incremental_filter_format`, `default_lookback_days`, `format_param`, `credentials`, `expected_field_count`) stay in YAML as typed-but-unconsumed documented intent — Pydantic models include them with `extra="forbid"` strictness, and `build_extractor_kwargs` excludes them from constructor kwargs via `model_fields` introspection. Future waves can wire individual fields without YAML schema changes by extending `to_extractor_kwargs` and the corresponding extractor field.

This deliberate choice avoids conflating the loader refactor with new extractor logic. Each YAML field's wire-or-defer decision was evaluated on functionality upside vs. lift; the unwired fields all scored low (filter param / format strings rarely change, env-var indirection through Settings already exists for credentials, etc.).

**Acceptance criterion met:** editing `config/sources/usda.yaml`'s `etag_enabled: true → false` takes effect on the next `recalls extract usda` run with no code change. The `usda.yaml` header that previously read "this file is NOT loaded by any code path" is gone — file is now the live kill switch. Same applies to `usda_establishments.yaml` (renamed from the singular `usda_establishment.yaml` so the filename matches the canonical source name in `extraction_runs.source`). The FDA `timeout_seconds: 60.0` discrepancy (where the YAML said 60s but runtime ran at the parent default of 30s) is resolved as a side effect.

**`--lookback-days` refactor.** Previously the CLI imported `_source_watermarks` from each extractor module and reached into `extractor._engine` (a `PrivateAttr`) to UPDATE the watermark cursor. Now `CpscExtractor` and `FdaExtractor` expose a public `override_watermark_lookback(days: int)` method; the CLI calls it generically. Encapsulates the engine + watermark-table access inside the extractor.

**Deep-rescan invariant.** `UsdaDeepRescanLoader.etag_enabled = False` is a class invariant (the deep-rescan loader bypasses ETag by design). The CLI's deep-rescan path passes `exclude=frozenset({"etag_enabled"})` to `build_extractor_kwargs` so a routine-path `etag_enabled: true` in YAML never overrides the invariant.

**Per-environment overlays — deferred.** ADR 0012 mentions per-env overlays as a possible benefit. The MVP is single-file-per-source: the loader reads exactly one YAML and validates through the discriminated union. Layered overlays (`<source>.<env>.yaml` merging or replacing into the base) are tracked as a Phase 7 prerequisite in `project_scope/implementation_plan.md` to land before production cron turns on. The current `Settings()` env-var indirection covers the legitimate per-env knobs today (DB URL, R2 keys, FDA creds); URL/timeout/etag values don't differ across dev and prod yet.
