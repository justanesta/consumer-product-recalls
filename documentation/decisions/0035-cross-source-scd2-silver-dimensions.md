...
- **Status:** Accepted — **ratified at the #59 merge** (which carried the PR 6b.5 USCG `firm_uscg_attributes` SCD-2 build), per the PR-merge status-flip convention (mirrors ADR 0036); status flip recorded 2026-06-06 (catch-up — the flip was missed at merge time). **Decision recorded 2026-06-05 on the A scope** (cross-source policy = Policy C; the only instance *built* is USCG `firm_uscg_attributes`). The Phase-6c follow-on refinements deferred in §5 (rename-vs-recycle tier, historical-interval backfill, as-of-build-date HIN join) are owned by `project_scope//archive/phase-6c-execution-plan.md` (commit 6c.5). The 2026-06-01 stub rationale and the 2026-06-02 applicability verdict are retained below as history. **Amended 2026-06-09 (C19):** the firm sidecars + their snapshots were renamed source-uniform — `firm_establishment_attributes → firm_usda_attributes`, `firm_manufacturer_attributes → firm_uscg_attributes` (+ `uscg_manufacturer_attributes_snapshot → firm_uscg_attributes_snapshot`); FDA was already uniform. Done inside the zero-cost window (snapshots held 0 edit-versions, so 0 history was lost — verified: re-snapshot `INSERT 0 0`, downstream `PASS=75`). The old snapshot tables are orphaned and dropped by the operator. References throughout this ADR use the new names.
...
- **USCG `firm_uscg_attributes` (`mic`) — Type-2-NEED, monitor-confirmed.** 221/718 OOB-recycled, 365/718 with any prior holder. The first cross-source SCD-2 instance and the only measured NEED.
...
### 3. Build scope **(this step) = USCG `firm_uscg_attributes` ONLY**
...
3. Build the snapshot + `_snapshots.yml`; repoint `firm_uscg_attributes.sql`; wire the bridge flag; add `assert_uscg_mic_reassignment_flag_present` + `assert_uscg_scd2_no_forked_lineage`; the two companion ADR edits (0031, 0007).
4. **User-run:** `dbt snapshot --select firm_uscg_attributes_snapshot` (banks current) → `dbt build --select firm_uscg_attributes recall_event_firm` → `dbt test` → re-run `dbt snapshot` on an unchanged corpus to confirm `strategy='check'` idempotency (0 spurious versions).
...
**Implication for the build decision:** prioritize `firm_uscg_attributes` (the confirmed NEED) + its as-of-build-date join; the BENEFIT dims are deferrable features whose monitors will quantify the payoff before the build commits. *(Settled 2026-06-05: build the USCG flag now; defer the as-of-build-date join + the BENEFIT dims.)*
...
- `dbt/snapshots/firm_usda_attributes_snapshot.sql` — anchor `establishment_number`; `check_cols` = the demographic/regulatory attributes EXCLUDING `latest_mpi_active_date` (heartbeat, ADR 0032). `firm_usda_attributes.sql` repointed to its `dbt_valid_to is null` current view (column contract unchanged → `recall_event_establishment_resolution` unaffected).
...
- **(a) rename-vs-recycle tier.** `firm_uscg_attributes.mic_renamed_not_recycled` = a prior
