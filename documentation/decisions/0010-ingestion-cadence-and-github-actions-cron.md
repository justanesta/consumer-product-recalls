...
| USCG manufacturer **details** | **quarterly** | The full ~14k-page detail sweep at the 1s polite throttle runs ~4.5–7.75h — it exceeds the GitHub-hosted 6h per-job cap, so it cannot be a single weekly job, and detail-only drift (a `Date Modified` bump with no listing change) moves slowly. Matches `project_scope/archive/phase-5d-uscg-manufacturers-detail.md`. A **1/12 monthly rotation** is recorded as a *future option*, not adopted: it needs a `--min-id`/`--max-id` range parameter on `deep-rescan` plus a persistent per-shard offset cursor (neither exists today); the simpler interim is the quarterly full sweep or the chunked-process driver of plan W9. **BUILT (Q6, 2026-06-09):** `--shard`/`--shard-count` param added; `deep-rescan-uscg-manufacturers-detail.yml` runs the monthly 1/3 shard (`timeout-minutes: 330`); stateless — shard index derived from calendar month (`(month-1) % shard_count`), no persistent cursor required (not `--min-id`/`--max-id`). Full corpus every quarter (3 monthly shards). The "future option" framing is superseded. WS-H must validate a real 1/3 shard completes under the GHA timeout before relying on the cron. |
...
`schedule:` triggers on extract and deep-rescan workflows remain **off** (workflow_dispatch-only) — Phase 7 C32 adds guarded triggers and C40 turns them on. `transform.yml` has a **live schedule trigger** (`0 3 * * *` — 03:00 UTC daily, added Phase 7) guarded by the `CRON_ENABLED` repo variable; it fires on schedule only when `CRON_ENABLED == 'true'`. These workflows are cron-*ready*.
