...
|---|---|---|---|---|---|
| **CPSC** | `RecallNumber` (stable upstream) | `md5(event_id\|name\|model\|product_ordinal)` per `recall_product.sql:31-46` | (a) `products[]` array reorder shifts ordinals → all later products fragment, (b) name/model character-level normalization | TBD — write parity assertion that snapshots `(event_id, name, model)` ↔ ordinal mapping over time | >0.1% silver row count fragmented per quarter |
| **FDA** | `RECALLEVENTID::text` (stable upstream) | `md5('FDA'\|PRODUCTID)` per `recall_product.sql:51` | PRODUCTID renumber if FDA changes its internal ID scheme | TBD — assert PRODUCTID stability across `extraction_timestamp` | Any non-zero rate (FDA stability is contractual) |
| **USDA** | `field_recall_number` (stable upstream) | `recall_product_id = recall_event_id` (1:1 — one product row per recall, ADR 0002 defers structured parsing of free-text `product_items`) | None at product level — single-row-per-recall grain. Free-text `product_items` lives in `source_specific_attrs`. | N/A — no product-level surrogate to fragment | N/A |
| **NHTSA** | `campno` (stable upstream — verified across both archives) | **`md5('NHTSA'\|campno\|maketxt\|modeltxt\|yeartxt\|compname\|rcl_cmpt_id\|mfr_comp_ptno\|mfr_comp_desc\|mfr_comp_name\|bgman\|endman)`** (11-tuple matching ADR 0030 bronze identity) | Cross-run drift on any of 11 identity fields. Empirical baseline 2026-05-08: 1 event/day rate (AC DELCO maketxt normalization) | Implemented — `assert_*_identity_stable.sql` + `cross_corpus_stability.py` | >0.01% silver row count fragmented per month, OR systematic drift on a previously-stable field |
...
- CPSC, FDA, USDA Tier 2 assertion scripts (per the per-source table)
