...
| **USCG** | `source_recall_id` (USCG recall id, stable upstream) | `recall_product_id = recall_event_id = md5('USCG'\|source_recall_id)` (1:1 — one product row per recall; USCG has no separate product breakdown) per `recall_product.sql:173-211` | None at product level — single-row-per-recall grain (mirrors USDA). The USCG-specific SCD concern is **firm-grain, not product-grain**: the `mic` firm anchor is RECYCLED across builders (a Type-2 NEED — `assert_mic_holder_stable.sql` = 221 OOB-recycled / 365 with any prior holder of 718 recalled MICs), handled by **[ADR 0035](0035-cross-source-scd2-silver-dimensions.md)**'s SCD-2 `firm_uscg_attributes`, not this product-fragmentation framework. | N/A for product fragmentation. Firm-anchor recycle monitor: `scripts/sql/cross_source/scd_monitors/assert_mic_holder_stable.sql` → feeds ADR 0035. | N/A (1:1 product grain; the firm-anchor SCD-2 ships per ADR 0035, PR 6b.5) |
