...
**What changed.** Layer 2 (the parallel prototype, per `silver_v15_migration_plan.md`) built the snapshot + `recall_product_v15` against the FULL NHTSA corpus for the first time — the original 6-tuple was cut on a dev slice plus the single Pierce event. The full-corpus diff (`scripts/sql/nhtsa/silver/compare_v1_v15_cardinality.sql`) showed the 6-tuple collapses `recall_product` (NHTSA) **321,540 → 194,377 rows (−127,163, −40%)**.
