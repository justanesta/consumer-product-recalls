...
- Several columns are **deliberately not normalized**, and the API models its response schemas around the raw shapes — "tidying" them silently would break it: `classification` is **source-native** (FDA `1`/`2`/`3`/`NC`, USDA `Class I`/`Class II`/`Class III`/`Public Health Alert`, USCG `H`/`L`/`M`/`S`, CPSC/NHTSA NULL — ⚠ **corrected 2026-W25**: this originally read "FDA/USDA `Class I/II/III`"; FDA actually serves the numeric codes `1`/`2`/`3`/`NC` per `stg_fda_recalls.yml` `accepted_values`, see `documentation/audit/silver_gold_provenance_audit_2026_w25.md`), not a unified enum; `risk_level` is USDA-only; `is_active` is **tri-state** (NULL for CPSC/NHTSA); `announced_at` is nullable by design; `distribution_states` (scalar text) is distinct from `distribution_state_codes` (`text[]`).
- `product_names`, `models`, and `hins` in `mart_recall_summary` are **always non-null arrays** (`'[]'::jsonb` via `coalesce` — `mart_recall_summary.sql:103-107`, labeled O1); the same guarantee applies to `firms`. The API models these fields as `list[...]`, never `list[...] | None`. Changing the coalesce to a bare `NULL`-propagating expression silently breaks API deserialization.
