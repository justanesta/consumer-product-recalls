...
> **Implemented (Phase 6d, amends the sketch above):** shipped as the `recalls re-ingest` **CLI command** + `src/bronze/reingest.py` core (following the `recover-rejected` precedent — core under `src/bronze/`, thin Typer command — not a loose `scripts/re_ingest.py`). The driver reads `extraction_runs` (successful runs with a landed `raw_landing_path` in the window, excluding prior rebaselines) rather than listing R2 prefixes — cleaner lineage, no new R2 API. `--change-type` is **required** and restricted to `{schema_rebaseline, hash_helper_rebaseline}`. Scope is the **5 JSON REST sources** (their `land_raw` writes `json.dumps(raw_records)`, inverted by `Extractor.parse_landed_payload` → `json.loads`); NHTSA/USCG are rejected with "use deep-rescan". Each payload is re-landed to a fresh R2 key under a new rebaseline `extraction_runs` row (so history joins correctly and the original routine run's change_type is untouched); the write goes through `BronzeLoader.from_contract(...)` directly — **not** the source's `load_bronze` (which would advance the freshness watermark) and **not** `run()` (which would write a USDA presence manifest and corrupt `recall_lifecycle`). Each replay run records `extraction_runs.replayed_from_run_id` (migration 0029) = the original run's id, so a re-ingest run is distinguishable from a normal `extract --change-type=schema_rebaseline` re-baseline (both are `schema_rebaseline`); this also lets re-ingest skip already-replayed originals (`--force` overrides) and lets `verify_reingest_lineage.sql` assert "no replay run wrote a presence manifest". Replayed bronze rows are stamped with the payload's **original** `extraction_timestamp` (via the source's bronze table), not `now()` — mirroring `recovery.seed_extraction_timestamp` — so replaying an older payload cannot leapfrog genuinely-newer bronze (e.g. a later re-baseline) in silver's `max(extraction_timestamp)` selection.
...
> **Implemented (Phase 6d):** shipped as `scripts/backfill_manifest.py`, **USDA-only** (the only `default_track_presence` source) and **census-first** — default mode is read-only and prints the backfillable count, the un-backfillable NULL-`run_id` tail, and the manifest floor; `--apply` performs the inserts (`ON CONFLICT DO NOTHING` on `uq_eri_identity`, skipping already-manifested runs). It reuses `reingest.replay_to_passing` (R2 bytes → passing records) + `manifest.build_presence_manifest_rows`, stamping each row with the run's **original** `run_id`. Runs with a NULL `run_id` (run_id was nullable from the baseline) are permanently un-backfillable — the manifest history has a hard floor, reported by the census.
...
- ~~**`scripts/re_ingest.py` (Mechanism B) and `scripts/backfill_manifest.py` (Mechanism C) are unimplemented.** Both are filed as Phase 6 deliverables in the implementation plan.~~ **Implemented in Phase 6d** — Mechanism B as the `recalls re-ingest` CLI (`src/bronze/reingest.py`; JSON sources only), Mechanism C as `scripts/backfill_manifest.py` (USDA-only, census-first). See the per-mechanism "Implemented" notes above and `project_scope/phase-6d-execution-plan.md`.
