...
> **Provenance:** renamed 2026-05-01 from `architecture_overview.md` (system-architecture content moved to `architecture.md`). Expanded from CPSC+FDA to all five recall-union sources 2026-06-03 (the `feature/silver-field-remap` W6 doc-sync), and the per-source field map relocated to the consolidation doc per the single-home rule.
...
`firm` is the **conformed dimension** — keyed on `normalized_name = upper(trim(name))`, shared across all five recall-union sources (CPSC, FDA, USDA, NHTSA, USCG — the four standalone extractors do not feed the firm dimension directly), and the only firm object the fact bridge (`recall_event_firm`) joins to (on `firm_id`). Matching on normalized name gives implicit cross-source dedup: a firm appearing in several sources collapses to one row with all its structured IDs gathered in `observed_company_ids`. `firm.sql` and `recall_event_firm.sql` must stay in **lockstep** (identical `firm_id` recipes per branch) — the relationships test catches any divergence as orphan `firm_id`s.
...
- **The `''`-club {FDA, USDA, NHTSA}** use empty string as the missing-value sentinel. FDA and USDA staging wrap optional columns in `nullif(col, '')`; **NHTSA staging does *not*** — only `model_year` is `nullif`'d (`stg_nhtsa_recalls.sql:65`), and its free-text (`desc_defect`, `corrective_action`, `conequence_defect`, `fmvss`, `title`, …) is normalized **downstream in the serving models** (`recall_event`/`recall_product`, `nullif(trim(...),'')`, the 2026-W25 empty-string fix), not at staging. A naive `IS NULL` against bronze misreads all three as present. (Provenance + the residual gaps this introduced: `documentation/audit/silver_gold_provenance_audit_2026_w25.md`.)
...
- **Two dimension tiers.** `first_seen_at` / `last_seen_at` / `edit_count` are **manifest-independent** — computed from bronze (`extraction_timestamp`, `count(distinct content_hash)`) for all five recall-union sources. `is_currently_active` / `was_ever_retracted` need the presence manifest (`extraction_run_identities`) and so come from the `track_presence` sources — **USDA + NHTSA** (C16); NULL for CPSC/FDA/USCG. NHTSA presence is **gated to the full-corpus deep-rescan runs** (those that write the manifest — `NhtsaDeepRescanLoader`, keyed on the loader, *not* a `change_type` filter), so it stays NULL until a deep-rescan/seed banks a complete manifest (WS-H).
...
- **NHTSA presence — enabled + gated (C16).** Presence ("is it still published") requires comparing each run's full identity set, which only the manifest captures — bronze can't substitute (content-hash dedup means an unchanged campno produces no recent bronze row, so "present-but-unchanged" is indistinguishable from "absent"). NHTSA *full-enumerates* **only on a deep-rescan** (both archives — written by `NhtsaDeepRescanLoader`, `writes_presence_manifest=True`); the daily incremental cron pulls POST_2010 only and writes no manifest. `recall_lifecycle` therefore **gates NHTSA presence to the deep-rescan runs that wrote the manifest** — keyed on the loader, *not* a `change_type` filter (the monthly scheduled deep-rescan refreshes presence even as `change_type='routine'`): until one banks a complete manifest (WS-H H-b), NHTSA presence is NULL — never wrong. The signal is **observed feed presence** (a campno dropped from / present in the source pull), not an authoritative agency retraction (interpreting "withdrawn on date X" is v2). CPSC/FDA/USCG stay off — FDA/USDA/USCG have a native status field, and CPSC has no status in its API at all (next bullet).
...
### 13. `recall_distribution_area` country expansion (C12, 2026-06-09)

The `recall_distribution_area` sidecar gained two new columns alongside the existing `distribution_state_codes[]`:

- `distribution_country_codes[]` (text array, ISO-3166-1 alpha-2) — parsed from FDA's international tail and USDA's comma-token text. The US is deliberately **not** stored here: the array is a clean foreign-only international-presence signal (US dominates at ~50,854 recalls and is already captured via `distribution_scope` + `distribution_state_codes`).
- `n_distribution_countries` (integer) — array length, null-safe.

**Grain expansion:** the model previously emitted only rows with `≥1 state`; after the FULL OUTER join on `recall_event_id` between the state aggregate and the country aggregate, the grain is `≥1 state OR ≥1 country`. A country-only recall (neither state nor scope = Nationwide) now gets a row with empty `distribution_state_codes[]`. The `fct_recalls_by_geography` state lens is inert to these rows (unnesting an empty array yields nothing).

**Seed:** a `country_iso` seed (~135 entries, excludes the US and "Georgia" to avoid the US-state collision) backs the parse. FDA's international tail is matched via a single `regexp_matches` alternation pass; USDA uses a comma-token match (yields ~0 today, symmetric for future data).

**Scale (prod 2026-06-09):** 273 truly-international-only recalls; ~2,740 country-but-no-state rows (2,467 nationwide-US + international, 273 intl-only); ~5,253 recalls already had a states row and just gain `distribution_country_codes[]`.

Field definitions for the sidecar are in `documentation/data_schemas.md`. The companion gold view `fct_recalls_by_country` is documented in `documentation/gold_design_notes.md`.

### 14. Quantity enrichment precision-scope + grain traps (C13, 2026-06-09)

`recall_product` gained four columns from a structured parse of the raw `number_of_units` free-text field: `quantity_value`, `quantity_unit`, `quantity_category` (count/weight/volume/grouping), and `quantity_basis` (`per_product` or `total_all_products`). Field definitions are in `documentation/data_schemas.md`.

**v1 precision scope.** The parser (`src/enrichment/quantity.py`) is deliberately precision-over-recall: it emits a value only for *clean single-quantity shapes* (~70% of rows yield a value+unit; ~17% yield value-only). The messy ~10% — lot IDs, list indices, code prefixes, N/M container compounds, multi-product blobs — is emitted as `NULL` and deferred to a v2 AI extractor (see `project_scope/freetext-enrichment-backlog.md`).

**Two grain traps** `fct_units_recalled` must avoid:

1. **`total_all_products` overcounting.** When `quantity_basis = 'total_all_products'`, the quantity is a recall-wide total that repeats on *every* product row for that recall. Summing it over product rows over-counts by N× (number of products). The aggregate uses `max(quantity_value)` per `(recall_event_id, quantity_category)` — exact because the value is constant across rows within a recall.

2. **`per_product` summing.** A `per_product` row IS the quantity for that specific product instance; summing across the recall's products is correct. The `quantity_basis` flag is the load-bearing discriminator — always filter or branch on it before aggregating.

**NHTSA/USCG.** NHTSA's `potaff` (vehicles potentially affected, a repeat across the 7-tuple component rows) is collapsed to `max` per `campno` before aggregation. USCG is 1:1 with `recall_event` — no fan-out. These two sources do not go through the quantity crosswalk; their `unit_count` is a direct integer cast.

### 15. USDA new silver columns: `terminated_year`, `reason_category_tokens`, `processing_categories` (C6/C14, 2026-06-09)

Three columns added to the USDA branches in silver, each with a distinct rationale:

**`terminated_year` (on `recall_event`, C6):** USDA's `field_closed_date` was dropped from the API response in June 2026 (Finding S). The year-grain fallback `closed_year` remains. `terminated_year` is `coalesce(closed_year::int, extract(year from closed_at)::int)` — a year-grain close flag for recalls where only the year (not the precise date) is known. `terminated_at` is kept as `closed_at` for older records that still carry the precise timestamp; for post-June 2026 closes, `terminated_at` is NULL and only `terminated_year` is populated. Consumers that need a close signal should `coalesce(terminated_at::date, make_date(terminated_year, 12, 31))` or equivalent.

**`reason_category_tokens` (on `recall_event`, C14):** USDA's `recall_reason` field is a comma-joined multi-value string (e.g. `"Undeclared Allergens, Misbranding"`). The silver column `reason_category_tokens` is a jsonb array of the individual tokens (`jsonb_agg(trim(t))`), derived from a `string_to_array(reason_category, ',')` split — USDA-only; all other sources' `reason_category` is a single value or NULL, so they get `NULL` tokens. jsonb was chosen over text[] for bronze consistency, containment filtering (`reason_category_tokens ? 'Unreported Allergens'`), and to avoid a dbt-postgres unit-test limitation with `cast(null as ARRAY)`.

**`processing_categories` (on `recall_product`, C14):** the same token-array pattern applied to USDA's `type` column (which maps to the multi-value `processing` field, e.g. `"Beef Products,Pork Products"`). The 10 processing tokens have no internal commas, so the comma-split is clean. Non-USDA sources' `type` is a single-value product-type code; they get `NULL` for `processing_categories`.

Both token arrays are guarded by dbt membership tests (`assert_usda_reason_tokens_in_taxonomy`, `assert_usda_processing_tokens_in_taxonomy`) — a returned row means an unknown token appeared in a source update.

### 16. USCG severity NULL-cast (2026-06-09)

`stg_uscg_recalls` now NULL-casts out-of-set severity values in silver while bronze keeps the raw value verbatim (ADR 0027). The staging column is:

```sql
case
    when upper(nullif(severity, '')) in ('H', 'L', 'M', 'S')
        then upper(nullif(severity, ''))
end as severity
```

One out-of-set value was observed in the 2026-06-09 corpus (probe: `scripts/sql/uscg_recalls/bronze/inspect_out_of_set_severity.sql`). The `accepted_values [H,L,M,S]` dbt test on the silver column now passes without a `warn` suppression, because `accepted_values` ignores NULLs. Bronze retains the raw string so provenance is preserved; if the out-of-set code's semantics are later determined (OII vs. USCG severity scale), the silver CASE expression can be extended without a rebaseline.
