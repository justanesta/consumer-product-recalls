...
> **Status (2026-06-09, Phase 7):** all new schema surfaces from Phase 7 are documented here — `dim_date`, `fct_recalls_by_country`, `recall_distribution_area` country columns, `recall_product` quantity fields, the three missed silver columns (`terminated_year`, `reason_category_tokens`, `processing_categories`), and the Phase 7 dbt tests. This file is the **single-home owner** for field-level definitions; design rationale and grain traps live in [`silver_design_notes.md`](silver_design_notes.md) and [`gold_design_notes.md`](gold_design_notes.md).
...
New singular tests added in Phase 7 (C12–C20). Tests in `dbt/tests/source_assumptions/` inherit `severity='warn'` from the directory config; others declare severity inline.
